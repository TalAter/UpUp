...
var CACHE_NAME="upup-cache";self.addEventListener("install",function(a){a.waitUntil(caches.open(CACHE_NAME).then(function(a){return a.put("sw-offline-content",new Response("You are offline"))}))}),self.addEventListener("message",function(a){"set-content"===a.data.action&&a.waitUntil(caches.open(CACHE_NAME).then(function(b){return b.put("sw-offline-content",new Response(a.data.content))}))}),self.addEventListener("fetch",function(a){a.respondWith(fetch(a.request.url)["catch"](function(){return caches.match("sw-offline-content")}))});